  ...
  *output_matrix = (matrix_t*)malloc(sizeof(matrix_t));
  ...
  for (uint32_t col = 0; col < flipped_b->cols; col++) {
    for (uint32_t row = 0; row < flipped_b->rows / 2; row++) {
        ...
        /*for (uint32_t row = 0; row < b_matrix->rows; row++) {
          for (uint32_t col = 0; col < b_matrix->cols; col++) {
            flipped_b->data[(b_matrix->rows - row - 1) * b_matrix->cols + (b_matrix->cols - col - 1)] = b_matrix->data[row * b_matrix->cols + col];
        }
    }*/
